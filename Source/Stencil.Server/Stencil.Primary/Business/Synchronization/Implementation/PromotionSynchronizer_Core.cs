...
{
    public partial class PromotionSynchronizer : SynchronizerBase<Guid>, IPromotionSynchronizer
    {
        public PromotionSynchronizer(IFoundation foundation)
            : base(foundation, "PromotionSynchronizer")
        {
        ...
        public override void PerformSynchronizationForItem(Guid primaryKey)
        {
            ...
            {
                Promotion domainModel = this.API.Direct.Promotions.GetById(primaryKey);
                if (domainModel != null)
                {
                    Action<Guid, bool, DateTime, string> synchronizationUpdateMethod = this.API.Direct.Promotions.SynchronizationUpdate;
                    if(this.API.Integration.SettingsResolver.IsHydrate())
                    {
                        synchronizationUpdateMethod = this.API.Direct.Promotions.SynchronizationHydrateUpdate;
                    }
                    ...
                    try
                    {
                        sdk.Promotion sdkModel = domainModel.ToSDKModel();
                        ...
                        {
                            this.API.Index.Promotions.DeleteDocument(sdkModel);
                            synchronizationUpdateMethod(domainModel.promotion_id, true, syncDate, null);
                        }
                        else
                        {
                            IndexResult result = this.API.Index.Promotions.UpdateDocument(sdkModel);
                            if (result.success)
                            {
                                synchronizationUpdateMethod(domainModel.promotion_id, true, syncDate, result.ToString());
                            }
                            else
                            {
        ...
        public override int PerformSynchronization(string requestedAgentName)
        {
            return base.ExecuteFunction("PerformSynchronization", delegate ()
            {
                ...
                if(this.API.Integration.SettingsResolver.IsHydrate())
                {
                    invalidItems = this.API.Direct.Promotions.SynchronizationHydrateGetInvalid(CommonAssumptions.INDEX_RETRY_THRESHOLD_SECONDS, agentName);
                }
                else
                {
                    invalidItems = this.API.Direct.Promotions.SynchronizationGetInvalid(CommonAssumptions.INDEX_RETRY_THRESHOLD_SECONDS, agentName);
                }
        ...
        /// </summary>
        protected void HydrateSDKModelComputed(Promotion domainModel, sdk.Promotion sdkModel)
        {
