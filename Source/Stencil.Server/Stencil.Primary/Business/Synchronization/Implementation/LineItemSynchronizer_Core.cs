...
{
    public partial class LineItemSynchronizer : SynchronizerBase<Guid>, ILineItemSynchronizer
    {
        public LineItemSynchronizer(IFoundation foundation)
            : base(foundation, "LineItemSynchronizer")
        {
        ...
        public override void PerformSynchronizationForItem(Guid primaryKey)
        {
            ...
            {
                LineItem domainModel = this.API.Direct.LineItems.GetById(primaryKey);
                if (domainModel != null)
                {
                    Action<Guid, bool, DateTime, string> synchronizationUpdateMethod = this.API.Direct.LineItems.SynchronizationUpdate;
                    if(this.API.Integration.SettingsResolver.IsHydrate())
                    {
                        synchronizationUpdateMethod = this.API.Direct.LineItems.SynchronizationHydrateUpdate;
                    }
                    ...
                    try
                    {
                        sdk.LineItem sdkModel = domainModel.ToSDKModel();
                        ...
                        {
                            this.API.Index.LineItems.DeleteDocument(sdkModel);
                            synchronizationUpdateMethod(domainModel.lineitem_id, true, syncDate, null);
                        }
                        else
                        {
                            IndexResult result = this.API.Index.LineItems.UpdateDocument(sdkModel);
                            if (result.success)
                            {
                                synchronizationUpdateMethod(domainModel.lineitem_id, true, syncDate, result.ToString());
                            }
                            else
                            {
        ...
        public override int PerformSynchronization(string requestedAgentName)
        {
            return base.ExecuteFunction("PerformSynchronization", delegate ()
            {
                ...
                if(this.API.Integration.SettingsResolver.IsHydrate())
                {
                    invalidItems = this.API.Direct.LineItems.SynchronizationHydrateGetInvalid(CommonAssumptions.INDEX_RETRY_THRESHOLD_SECONDS, agentName);
                }
                else
                {
                    invalidItems = this.API.Direct.LineItems.SynchronizationGetInvalid(CommonAssumptions.INDEX_RETRY_THRESHOLD_SECONDS, agentName);
                }
        ...
        /// </summary>
        protected void HydrateSDKModelComputed(LineItem domainModel, sdk.LineItem sdkModel)
        {
